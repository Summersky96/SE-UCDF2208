...
<?php
    ...
    $sql = "SELECT * FROM mechanicinfo WHERE mecid = $userId";
    ...
    if ($result) {
        ...
        if (isset($_POST['submitBtn'])) {
            ...
            $updateSql = "UPDATE mechanicinfo SET name = '$name', email = '$email', telephone = '$telephone', mecpassword = '$password' WHERE mecid = $userId";
        ...
        if (isset($_POST['deleteBtn'])) {
            ...
            $deleteSql = "DELETE FROM mechanicinfo WHERE mecid = $userId";
            if (mysqli_query($con, $deleteSql)) {
                // Account deleted successfully
                echo '<script>alert("Account deleted successfully!");window.location.href="goodbye.php";</script>';
                session_destroy(); // Destroy the session
                // header("Location: goodbye.php"); // Redirect to a farewell page or login page
                exit();
            } else {
...
?>
    ...
    <section>
        ...
        <div class="box">
            <div class="form">
                <form action="mecinfo.php" method="post" enctype="multipart/form-data">
                    <h2>Edit Profile</h2>
                    <?php if(isset($row)): ?>
                    <input type="hidden" name="id" value="<?php echo $row['mecid']?>">
                    <div class="profilepic" id="profile-pic-container">
                        ...
                        <img id="profile_pic" src="<?php echo $row['mecpic'] ?>" alt="Profile Picture">
                    </div>
                    <div class="inputbox">
                    ...
                    <div class="inputbox">
                        ...
                        <input type="password" name ="password" id="passcode" required value="<?php echo$row['mecpassword']?>">
            ...
            <form action="mecupdate.php" method="post">
                <input type="hidden" name="id" value="<?php echo $row['mecid']; ?>">
                <button type="submit" name="deleteBtn" class="btn deactivate-btn" onclick="return confirm('Are you sure you want to Deactivate your account?');">Delete Account</button>         
            </form>
            </div>
        </div>
    </section>
<script>
        ...
        function resetProfilePicture() {
            ...
            var defaultSrc = '<?php echo $row['mecpic'] ?>'; // Default image source
